...
class MakanSiangFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_makan_siang, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        titleTextView.text = "Makan Siang"
        ...
        recyclerView = view.findViewById(R.id.rvMakanSiang)
        recyclerView.layoutManager = LinearLayoutManager(requireContext())

        fetchDataMakanSiang()
    }

    private fun fetchDataMakanSiang() {
        val uid = auth.currentUser?.uid ?: return
        val dateFormat = SimpleDateFormat("EEEE, dd MMMM yyyy", Locale("id"))
        val currentDate = dateFormat.format(Calendar.getInstance().time)
        val path = "users/$uid/history/$currentDate/Makan Siang"
    ...
    private fun fetchUserData(
        ...
        userRef.get().addOnSuccessListener { document ->
            if (document != null && document.exists()) {
                ...
                view?.apply {
                    findViewById<TextView>(R.id.tvKaloriSiang).text =
                        "${String.format("%.1f", totalKalori)}/${String.format("%.1f", maksimalKalori)}g"
                    findViewById<TextView>(R.id.tvKarboSiang).text =
                        "${String.format("%.1f", totalKarbo)}/${String.format("%.1f", maksimalKarbo)}g"
                    findViewById<TextView>(R.id.tvProteinSiang).text =
                        "${String.format("%.1f", totalProtein)}/${String.format("%.1f", maksimalProtein)}g"
                    findViewById<TextView>(R.id.tvLemakSiang).text =
                        "${String.format("%.1f", totalLemak)}/${String.format("%.1f", maksimalLemak)}g"

                    findViewById<ProgressBar>(R.id.progressKaloriSiang).progress = persenKalori
                    findViewById<ProgressBar>(R.id.progressKarboSiang).progress = persenKarbo
                    findViewById<ProgressBar>(R.id.progressProteinSiang).progress = persenProtein
                    findViewById<ProgressBar>(R.id.progressLemakSiang).progress = persenLemak
                }
            }
        }.addOnFailureListener {
