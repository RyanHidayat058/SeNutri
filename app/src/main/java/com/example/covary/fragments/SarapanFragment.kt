...
class SarapanFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_sarapan, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        recyclerView = view.findViewById(R.id.rvSarapan)
        recyclerView.layoutManager = LinearLayoutManager(requireContext())

        fetchDataSarapan()
    }

    private fun fetchDataSarapan() {
        val uid = auth.currentUser?.uid ?: return
        val dateFormat = SimpleDateFormat("EEEE, dd MMMM yyyy", Locale("id"))
        val currentDate = dateFormat.format(Calendar.getInstance().time)
        val path = "users/$uid/history/$currentDate/Sarapan"
    ...
    private fun fetchUserData(
        ...
        userRef.get().addOnSuccessListener { document ->
            if (document != null && document.exists()) {
                ...
                view?.apply {
                    findViewById<TextView>(R.id.tvKaloriSarapan).text =
                        "${String.format("%.1f", totalKalori)}/${String.format("%.1f", maksimalKalori)}g"
                    findViewById<TextView>(R.id.tvKarboSarapan).text =
                        "${String.format("%.1f", totalKarbo)}/${String.format("%.1f", maksimalKarbo)}g"
                    findViewById<TextView>(R.id.tvProteinSarapan).text =
                        "${String.format("%.1f", totalProtein)}/${String.format("%.1f", maksimalProtein)}g"
                    findViewById<TextView>(R.id.tvLemakSarapan).text =
                        "${String.format("%.1f", totalLemak)}/${String.format("%.1f", maksimalLemak)}g"

                    findViewById<ProgressBar>(R.id.progressKaloriSarapan).progress = persenKalori
                    findViewById<ProgressBar>(R.id.progressKarboSarapan).progress = persenKarbo
                    findViewById<ProgressBar>(R.id.progressProteinSarapan).progress = persenProtein
                    findViewById<ProgressBar>(R.id.progressLemakSarapan).progress = persenLemak
                }
            }
        }.addOnFailureListener {
